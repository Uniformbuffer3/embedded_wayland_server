...
fn create_server(){
    ...
    parameters.drm_formats = vec![
        DrmFormat{code: DrmFourcc::Abgr8888,modifier: DrmModifier::Linear},
    ...
    server.add_cursor(0);
    server.create_output(1,"Outpu1", PhysicalProperties{
        size: (1920,1080).into(),
            ...
            match request{
                    ...
                    match request {
                        XdgRequest::NewToplevel{surface}=>{
                            ...
                            with_states(&surface.get_surface().unwrap(),|surface_data|{
                                ...
                                println!("Committing {:#?}: {:#?}",surface,surface_data.cached_state.pending::<SurfaceAttributes>());
                ...
                WaylandRequest::Commit{surface}=>{
                    with_states(&surface,|surface_data|{
                        ...
                        surface_attributes.buffer.as_ref().map(|buffer|{
                            match buffer {
